...
class Crudx 
{
	...
	*/
	...
	protected  $password="janina@password";
	protected  $database="crudx";
...
/*constructor for Crud class
	...
	{
		$this->host = is_null($host) ? $this->host : $host;
		$this->username = is_null($user) ? $this->username : $user;
		$this->password = is_null($pwd) ? $this->password : $pwd;
		$this->database = is_null($db) ? $this->database : $db;

		// connection for database server by using username and password

		$this->_db = new \mysqli($this->host, $this->username, $this->password, $this->database);
		...
		$this->_db->query("SET SESSION collation_connection ='utf8_unicode_ci'");
	}
